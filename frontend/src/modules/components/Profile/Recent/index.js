...
class Recent extends Component {
    render() {
        ...
        //
        const last_entries = this.props.recent.length > 0 ? this.props.recent : []
        // TODO: Reverse order

        return (
            <Grid>
            <Grid.Row columns={4}>
                {last_entries.map(entry =><Grid.Column key={entry.id}> <MovieCard movie={entry.movie}/> </Grid.Column>)}
            </Grid.Row>
            </Grid>
        );
    }
